...
export function ThankYouPageClient() {
  ...
  const loadCheckoutData = async () => {
    try {
      ...
      logger.debug('ThankYouPage: Received checkout data', {
        productId: checkoutData.product?.id,
        hasDescription: !!checkoutData.product?.description,
        hasHighlights: !!checkoutData.product?.highlights,
        highlightsLength: checkoutData.product?.highlights?.length || 0,
        hasIncludedItems: !!checkoutData.product?.included_items,
        includedItemsLength: checkoutData.product?.included_items?.length || 0,
        hasExcludedItems: !!checkoutData.product?.excluded_items,
        excludedItemsLength: checkoutData.product?.excluded_items?.length || 0,
        hasMeetingInfo: !!checkoutData.product?.meeting_info,
        hasCancellationPolicy: !!checkoutData.product?.cancellation_policy,
        productData: checkoutData.product
      });
  ...
  const normalizeImageUrl = (url: string | null | undefined): string => {
    ...
    // Se è HTTP, convertilo in HTTPS o usa placeholder
    if (url.startsWith('http://')) {
      return url.replace('http://', 'https://');
    }
    
    // Se inizia con /, potrebbe essere un path relativo - non supportato per immagini esterne
    if (url.startsWith('/')) {
    ...
    // Formato: storage/v1/object/public/bucket/path o bucket/path
    const supabaseConfig = getSupabaseConfig();
      ...
      <main>
        ...
        <div style={{ 
          ...
          <div className="thank-you-product-card" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="thank-you-product-image" style={{ 
              ...
              <ImageWithFallback
                src={productImage}
                alt={data.product.name || 'Immagine prodotto'}
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {!((data.product.no_adults === true) && data.booking.numberOfAdults === 0) && (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                <div style={{ marginTop: '4px', flexShrink: 0 }}>
