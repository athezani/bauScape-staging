...
import { getStripeCheckoutUrl } from '../utils/env';
import { getSupabaseClient } from '../lib/supabaseClient';
import { pricingService } from '../services/pricingService';
import { logger } from '../utils/logger';
import '../styles/program-list.css';

const ProductImageCarousel = lazy(() => import('../components/ProductImageCarousel').then(m => ({ default: m.ProductImageCarousel })));
const AvailabilitySelector = lazy(() => import('../components/AvailabilitySelector').then(m => ({ default: m.AvailabilitySelector })));
const ProductAttributes = lazy(() => import('../components/ProductAttributes').then(m => ({ default: m.ProductAttributes })));
...
export function ProductDetailPage({ product, onBack, onNavigate }: ProductDetailPageProps) {
  ...
  useEffect(() => {
    if (product.type === 'trip' && !selectedSlotId) {
      ...
      loadTripData();
    } else if (product.type === 'trip' && product.start_date) {
  ...
  }, [product.type, product.id, selectedSlotId, product.start_date, product.end_date, product.durationDays]);
    ...
    const loadSlotAvailability = async () => {
      ...
      try {
        ...
        if (slotData) {
          ...
          const isNoAdults = (product.noAdults === true || product.noAdults === 1) && (product.type === 'class' || product.type === 'experience');
  ...
  return (
    <div className="min-h-screen bg-white">
      <SEOHead
        title={product.title}
        description={product.description || `Prenota ${product.title} - ${product.location || 'Esperienza unica'} per te e il tuo cane. Scopri dettagli, prezzi e disponibilità.`}
      ...
      <div className="max-w-[1200px] mx-auto px-4 mb-8">
        <Suspense fallback={<div className="w-full h-[400px] bg-gray-100 animate-pulse rounded-3xl"></div>}>
            ...
            >
              ...
              {isSoldOut ? (
                ...
                <>
                  ...
                  {!((product.noAdults === true || product.noAdults === 1) && (product.type === 'class' || product.type === 'experience')) && (
                    <div className="mb-6">
                  ...
                  <div className="mb-6 pb-6 border-b border-gray-100">
                    {(() => {
                      ...
                      const showAdults = !((product.noAdults === true || product.noAdults === 1) && (product.type === 'class' || product.type === 'experience'));
             ...
             <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 shadow-lg lg:hidden z-40">
               <div className="flex items-center justify-between gap-4">
                 <div>
                   ...
                   {selectedDate && (
                     <small style={{ color: 'var(--text-gray)' }}>
                       {!((product.noAdults === true || product.noAdults === 1) && (product.type === 'class' || product.type === 'experience')) && `${guests} ${guests === 1 ? 'persona' : 'persone'}`}
                       {!((product.noAdults === true || product.noAdults === 1) && (product.type === 'class' || product.type === 'experience')) && ' • '}
                       {dogs} {dogs === 1 ? 'cane' : 'cani'}
                     </small>
                   )}
