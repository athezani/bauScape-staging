/**
 * Script Automatico di Test Booking
 * Crea booking di test per tutte e 3 le tipologie e verifica idempotency_key
 */
...
async function createTestBooking(productType) {
  ...
  try {
    ...
    if (productError || !products || products.length === 0) {
      ...
      if (productType === 'trip') {
        productData.start_date = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        productData.end_date = new Date(Date.now() + 37 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
      }
    ...
    const { data: booking, error: fetchError } = await supabase
      .from('booking')
      .select('id, idempotency_key, order_number')
      .eq('id', bookingId)
      .single();
