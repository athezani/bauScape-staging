-- ============================================
-- Update create_booking_transactional to include product_id
-- ============================================
...
CREATE OR REPLACE FUNCTION public.create_booking_transactional(
  ...
  p_product_type TEXT,
  p_product_id UUID,  -- ✅ NEW: Added product_id as required
  p_provider_id UUID,
  p_booking_date DATE,
...
BEGIN
  ...
  IF p_product_id IS NULL THEN
    RETURN QUERY SELECT NULL::UUID, FALSE, 'Product ID is required and cannot be NULL'::TEXT;
    RETURN;
  END IF;
  ...
  -- Create booking with product_id
...
COMMENT ON FUNCTION public.create_booking_transactional IS 
  'Atomically creates a booking with product_id and decrements availability. Returns booking_id on success, error on failure.';
