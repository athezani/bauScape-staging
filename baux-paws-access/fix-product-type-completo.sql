-- Fix COMPLETO: Aggiorna funzione transazionale con cast ENUM corretto
-- IMPORTANTE: Esegui questo script nel SQL Editor di Supabase

-- Prima verifica che la funzione esista
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Funzione create_booking_transactional non trovata. Applica prima la migrazione principale.';
  END IF;
END $$;

-- Aggiorna la funzione con cast esplicito
...
DECLARE
  ...
  v_error_msg TEXT;
  v_product_type_enum public.product_type;  -- Variabile enum per il cast
BEGIN
  ...
  -- Validate and cast product_type to enum
  BEGIN
    v_product_type_enum := p_product_type::public.product_type;
  EXCEPTION
    WHEN invalid_text_representation THEN
      RETURN QUERY SELECT NULL::UUID, FALSE, format('Invalid product_type: %s. Must be one of: experience, class, trip', p_product_type)::TEXT;
      RETURN;
  END;
  ...
  -- Create booking with enum variable (already cast)
  INSERT INTO public.booking (
    ...
    p_idempotency_key,
    v_product_type_enum,  -- Usa la variabile enum già castata
    p_provider_id,
...
EXCEPTION
  ...
  WHEN invalid_text_representation THEN
    ...
    RETURN QUERY SELECT NULL::UUID, FALSE, format('Invalid product_type: %s. Error: %s', p_product_type, v_error_msg)::TEXT;
  WHEN OTHERS THEN
...
BEGIN
  IF EXISTS (
    ...
    WHERE routine_name = 'create_booking_transactional'
    AND routine_schema = 'public'
    AND routine_definition LIKE '%v_product_type_enum%'
  ) THEN
    RAISE NOTICE '✅ Fix applicato correttamente!';
  ELSE
    RAISE WARNING '⚠️  Verifica che il fix sia stato applicato correttamente';
  END IF;
END $$;

COMMENT ON FUNCTION public.create_booking_transactional IS 
  'Atomically creates a booking and decrements availability. Returns booking_id on success, error on failure. Validates idempotency_key and casts product_type to enum.';
