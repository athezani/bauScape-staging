-- Fix: Aggiorna funzione transazionale per cast corretto di product_type
-- Esegui questo script per correggere il problema del tipo ENUM
...
BEGIN
  ...
  IF p_product_type NOT IN ('experience', 'class', 'trip') THEN
  ...
  -- Create booking with explicit enum cast
  INSERT INTO public.booking (
    ...
    p_idempotency_key,
    p_product_type::public.product_type,  -- CRITICAL: Cast to enum type
    p_provider_id,
...
COMMENT ON FUNCTION public.create_booking_transactional IS 
  'Atomically creates a booking and decrements availability. Returns booking_id on success, error on failure. Validates idempotency_key is not NULL and product_type is valid enum.';
