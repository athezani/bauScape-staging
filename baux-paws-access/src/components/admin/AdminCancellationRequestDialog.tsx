...
interface AdminCancellationRequestDialogProps {
...
export const AdminCancellationRequestDialog = ({
  request,
  open,
  onOpenChange,
  onProcessed,
}: AdminCancellationRequestDialogProps) => {
  ...
  // Load cancellation policy from product when dialog opens
  useEffect(() => {
    const loadCancellationPolicy = async () => {
      if (!request?.booking?.product_type || !request?.booking?.product_id) {
        setCancellationPolicy(null);
        return;
      }

      try {
        const tableName = request.booking.product_type === 'trip' ? 'trip' : 
                         request.booking.product_type === 'experience' ? 'experience' : 'class';
        
        const { data, error } = await supabase
          .from(tableName)
          .select('cancellation_policy')
          .eq('id', request.booking.product_id)
          .single();

        if (!error && data) {
          setCancellationPolicy(data.cancellation_policy || null);
        } else {
          setCancellationPolicy(null);
        }
      } catch (error) {
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <div className="text-sm font-medium">{request.booking.provider.company_name}</div>
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Contatto</Label>
                  <div className="text-sm">{request.booking.provider.contact_name}</div>
                </div>
              </div>
            </div>
          )}
